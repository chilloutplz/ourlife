...
class _NoteEditScreenState extends State<NoteEditScreen> {
  ...
  void initState() {
    ...
    if (widget.note?.biblePassage != null) {
      try {
            ...
            widget.note!.biblePassage!
                ...
                .map((s) {
                  try {
                    ...
                    debugPrint(
                      'Error parsing individual passage: $s, error: $e',
                    );
                    return null;
                  }
                })
        ...
        debugPrint(
          'Error parsing biblePassage: ${widget.note!.biblePassage}, error: $e',
        );
      }
    }
  ...
  Future<void> _loadSidePanelFontSize() async {
    ...
    setState(() {
      _sidePanelFontSize =
          prefs.getDouble('noteEditScreen_sidePanelFontSize') ?? 14.0;
    });
  }
  ...
  Future<void> _saveSidePanelFontSize() async {
    ...
    await prefs.setDouble(
      'noteEditScreen_sidePanelFontSize',
      _sidePanelFontSize,
    );
  }
  ...
  void _openBiblePassageModal() {
    showDialog(
      context: context,
      builder:
          (dialogContext) => BiblePassageSelector(
            ...
            onPassageSelected: (passage) async {
              ...
              try {
                ...
                final selectedVersions =
                    prefs.getStringList('selected_versions') ?? ['우리말'];
                ...
                for (var versionName in selectedVersions) {
                  ...
                  for (
                    int chap = passage.startChap;
                    chap <= passage.endChap;
                    chap++
                  ) {
                    final chapterVerses = await BibleService.getVerses(
                      versionName,
                      passage.book,
                      chap,
                    );
                  ...
                  fetchedVersesForRangeByVersion[versionName] =
                      aggregatedVersesForThisVersion;
                }

                if (!mounted) return;

                final firstSelectedVersionName = selectedVersions.first;
                final List<Verse> referenceVerseList =
                    fetchedVersesForRangeByVersion[firstSelectedVersionName] ??
                    [];

                final int actualStartIndex = referenceVerseList.indexWhere(
                  (v) =>
                      v.chapter == passage.startChap &&
                      v.number == passage.startVer,
                );
                final int actualEndIndex = referenceVerseList.indexWhere(
                  (v) =>
                      v.chapter == passage.endChap &&
                      v.number == passage.endVer,
                );
                ...
                if (actualStartIndex != -1 &&
                  ...
                  final books = await BibleService.getBooks(
                    firstSelectedVersionName,
                  );
                  final bookName =
                      books
                          .firstWhere((book) => book.slug == passage.book)
                          .name;

                  String chapterDisplay =
                      passage.startChap == passage.endChap
                          ? '${passage.startChap}장'
                  ...
                  for (int i = actualStartIndex; i <= actualEndIndex; i++) {
                    ...
                    for (var versionName in selectedVersions) {
                      final List<Verse> versesOfThisVersion =
                          fetchedVersesForRangeByVersion[versionName] ?? [];
                      ...
                      if (actualVerseToDisplay != null) {
                        verseSpans.add(
                          TextSpan(
                            children: [
                              if (isFirstVersionForThisVerse)
                                TextSpan(
                                  text: '$currentVerseNum. ',
                                  style: TextStyle(
                                    color: Colors.blue,
                                    fontWeight: FontWeight.bold,
                                    fontSize: _sidePanelFontSize, // 본문 번호의 폰트 크기
                                  ),
                                ),
                              // TextSpan(
                              //   text: '첫번째 버전명 ',
                              //   style: const TextStyle(
                              //     color: BootstrapColors.info, // 색상 설정
                              //     fontStyle: FontStyle.italic,
                              //     fontSize: 10, // 고정된 폰트 크기
                              //   ),
                              // ),
                              TextSpan(
                                ...
                                style: TextStyle(
                                  color: Colors.white,
                                  fontSize: _sidePanelFontSize, // 본문의 폰트 크기
                                ),
                              ),
                            ],
                          ),
  ...
  Future<void> _loadPassageContents(List<Passage> passagesToLoad) async {
    try {
      ...
      for (
        int passageIdx = 0;
        passageIdx < passagesToLoad.length;
        passageIdx++
      ) {
        ...
        if (actualStartIndex != -1 &&
          ...
          final bookName =
              books.firstWhere((book) => book.slug == passage.book).name;
          ...
          if (!mounted) return;
          setState(() {
            final indexInState = _passages.indexWhere(
              (p) =>
                  p.book == passage.book &&
                  p.startChap == passage.startChap &&
                  p.startVer == passage.startVer &&
                  p.endChap == passage.endChap &&
                  p.endVer == passage.endVer,
            );
            if (indexInState != -1) {
  ...
  TextSpan _applyFontSizeToTextSpan(
    ...
    if (isTitle) {
      ...
      newStyle = newStyle.copyWith(
        ...
        color: newStyle.color ?? Colors.white, // 기존 색상이 없으면 기본값 설정
      );

      // 특정 텍스트 조건에 따른 스타일 변경
      if (originalSpan.text != null) {
        final text = originalSpan.text!.trim();
        if (text.endsWith(' ') && int.tryParse(text) != null) {
          // 구절 번호 스타일
          newStyle = newStyle.copyWith(
            color: Colors.blue,
            fontWeight: FontWeight.bold,
  ...
  Widget _buildSectionLabel(String text, Color textColor) {
    return Text(text, style: TextStyle(fontSize: 12, color: textColor));
  }
}
                            ...
                            DropdownButtonFormField<int>(
                              ...
                              onChanged: (v) {
                                if (v != null) {
                                  ...
                                  setState(() {
                                    _selectedVerse = v;
                                    if (_selectedEndChapter ==
                                        _selectedChapter) {
                                      _selectedEndVerse = v; // 끝 절을 시작 절과 동일하게 설정
                                    }
                                  });
                                }
                              },
                                ...
                                if (v != null) {
                                  ...
                                  setState(() {
                                    ...
                                    if (_selectedEndChapter !=
                                      ...
                                      if (_selectedEndVerse < _selectedVerse)
...
class Passage {
  ...
  factory Passage.fromString(String s) {
    ...
    try {
      ...
      if (parts.length < 2) {
        debugPrint(
          'Invalid passage format: $s (not enough parts, needs at least 2)',
        );
      ...
      if (rangeString.contains('~')) {
        ...
        final startParts =
            ranges[0].split(':').map(int.parse).toList(); // 시작 장절
